package io.github.pixix4.kobserve.list
...
class FilteredObservableListTest {
    ...
    fun setup() {
        ...
        observableList = observableSourceList.filterObservable { it < 3 }
        reconstructionList = ReconstructionList(observableList)
    }

    @Test
    fun propertyTest() {
        val t1 = property { it: Int -> it < 3 }
        val s1 = observableSourceList.filterObservable(t1)
        
        val t2 = property(true)
        val s2 = observableSourceList.filterObservable(t2) { element, predicate ->
            if (predicate) element < 3 else element > 3
        }

        assertEquals(listOf(1, 2), s1)
        assertEquals(listOf(1, 2), s2)

        t1.value = { it: Int -> it > 3 }
        t2.value = false

        assertEquals(listOf(4, 5), s1)
        assertEquals(listOf(4, 5), s2)
    }
    ...
    fun addAllTest() {
        ...
        assertEquals<List<Int>>(listOf(1, 2, -6, 1, 2, -6), observableList)
    }
    ...
    fun addAllIndexTest() {
        ...
        assertEquals<List<Int>>(listOf(1, 2, -5, -7, 1, 2), observableList)
    }
    ...
    fun setTest() {
        ...
        assertEquals(5, observableSourceList.set(2, -1))
        assertEquals(-1, observableSourceList.set(2, -2))
        assertEquals<List<Int>>(listOf(1, 2, -2), observableList)

        assertEquals(-2, observableSourceList.set(2, 22))
        assertEquals<List<Int>>(listOf(1, 2), observableList)
    }
